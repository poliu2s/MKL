...
bool readMatrixFromFile(vcl_string filename, int* m, int m_rows, int m_cols)
{
...
bool readMatrixFromFile(vcl_string filename, double* m, int m_rows, int m_cols)
{
	vcl_ifstream inFile(filename.c_str());
	vcl_string dummy;
	if(!m)
...
{	
	...
	model.readModel("C:/MultiObjModelToUSReg/data/EuclideanModel.txt");
	...
	double* shapeCoef; //= (double*)mkl_malloc(2 * 1 * sizeof(double), 64);
