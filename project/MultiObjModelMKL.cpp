...
void MultiObjModelMKL::readModel(vcl_string filename)
{
...
void MultiObjModelMKL::writePolyDataInVTK(double* vertices, int vertices_rows, int D, double* faces, int faces_rows, string fileName) {
	
	const char * fileNameInChar = fileName.c_str();

	ofstream outFile(fileNameInChar);
	if (outFile.is_open()){
		outFile << "# vtk DataFile Version 3.0\nvtk output \nASCII \nDATASET POLYDATA \nPOINTS " << vertices_rows << " float\n";
		
		for (int i = 0; i < vertices_rows; i++) {
			for (int j = 0; j < D; j++) {
				outFile << vertices[i*D + j] << " ";
			}
			outFile << endl;
		}

		outFile << "\nPOLYGONS " << faces_rows << " " << faces_rows*4 << endl;
		
		
		//faces_expanded->set_column(0, 3);
		//faces_expanded->set_column(1, faces->apply(sub1_function).get_column(0));
		//faces_expanded->set_column(2, faces->apply(sub1_function).get_column(1));
		//faces_expanded->set_column(3, faces->apply(sub1_function).get_column(2));
		//outFile << *faces_expanded << endl;
		for(int i = 0; i < faces_rows; i++) {
			outFile << 3 << " ";

			for(int j = 0; j < D; j++) {
				outFile << faces[i*3 + j]-1.0 << " ";
			}
			outFile << endl;
		}
			
		outFile << "\nCELL_DATA " << faces_rows << " \nPOINT_DATA " << vertices_rows;
	}
